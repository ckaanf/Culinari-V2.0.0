	...
	useEffect(() => {
		...
		const getCategoryData = async () => {
			const { data } = await axios.get(
				...
				}/category/categorydetail/${code.slice(0, 3)}`
			);
		...
		<Container>
			<PageHeader>
				{location.search ? (
					<div>
						'
						<span className="search_keyword">
							{decodeURIComponent(location.search.slice(9))}
						</span>
						'에 대한 검색결과
					</div>
				) : (
					<>
						<div>
							{categoryData.data && categoryData.data[0].name}
						</div>
						<ul className="category_details">
							<CategoryList
								...
								datacode={
									categoryData.data &&
									categoryData.data[0].categoryCode
								}
							>
								<Link
									to={`/category/${
							...
							{categoryData.data &&
								categoryData.data[0].categoryDetails.map(
									(category, index) => {
										return (
											<CategoryList
												code={code}
												datacode={
													category.categoryDetailCode
												}
												key={index}
											>
												<Link
			...
			<div className="product_list_header">
				<div className="product_list_count">{`총 ${
