...
function Collection() {
	...
	useEffect(() => {
		const query = {
			sorted_type: sort,
			filter: checkedCategoryCodes.length
				? `category%3A${checkedCategoryCodes.join("%2C")}`
				: null,
			size: 20,
			page: currentPage,
		};
		...
		const getData = async () => {
			try {
				const { data } = await axios.get(
					`${process.env.REACT_APP_URL}/collections/${params}?${queryString}`
				);
